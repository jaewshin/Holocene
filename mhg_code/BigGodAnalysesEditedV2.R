...
#MHP: I added this code to print out the times going into the Nature analysis (for MoralisingGods and DoctrinalMode)
for(i in 1:length(NGAs)){
  dt <- dat[dat$NGA == NGAs[i],]
  print('----')
  print(NGAs[i])
  print('--')
  print('Time and MoralisingGod')
  print(dt[,c('Time','MoralisingGods','DoctrinalMode')])
} 
write.csv(dat,'data_used_for_nature_analysis.csv')

#Overall rate (beginning to end of polity)
## MHP: If dt$MoralisingGods is missing (NA), rates is empty in the following for loop
## Hence, all observations with missing data are removed within this for loop
## 10 observations disappear here, with missing data being treated as no MHG
## In particular, the following ten NGAs are removed:
## [1] "Cahokia"              "Cuzco"                "Finger Lakes"        
## [4] "Garo Hills"           "Lena River Valley"    "Lowland Andes"       
## [7] "North Colombia"       "Oro PNG"              "Southern China Hills"
## [10] "Valley of Oaxaca" 
...
## MHP: The following line drops 199 rows, leaving 200. Really, the following removes entries
## with NA in the sixth column of out since no non missing entries are greater than 1000
## The following 8 NGAs get eliminated at this step:
## [1] "Big Island Hawaii"    "Cambodian Basin"      "Central Java"        
## [4] "Chuuk Islands"        "Ghanaian Coast"       "Iceland"             
## [7] "Kapuasi Basin"        "Yemeni Coastal Plain"
## Big Island Hawaii: Appears in last observation, 1800 AD, and is NA before
## Cambodian Basin: Appears for all observations (300 to 1800 AD)
## Central Java: Appears for all observations (800 to 1700 AD)
## Chuuk Islands: Appears for all observations (1800 to 1900 AD)
## Ghanaian Coast: Appears for all observations (1600 to 1800 AD)
## Iceland: Appears for all observations (1000 to 1300 AD)
## Kapuasi Basin: Appears for all observations (1700 to 1900 AD)
## Yemeni Coastal Plain: Appears for all observations (400 to 1800 AD)
out <-subset(out, out[,6]<1000) #Removing windows without matching pre-/post-MG rates
write.csv(out, file="FullRates.csv",  row.names=FALSE)
out <- read.table("FullRates.csv", sep=",", header=TRUE)
NGAs <- levels(out$NGA)

## MHP: At this point the following 12 NGAs are left:
## [1] "Deccan"                     "Kachi Plain"               
## [3] "Kansai"                     "Konya Plain"               
## [5] "Latium"                     "Middle Yellow River Valley"
## [7] "Niger Inland Delta"         "Orkhon Valley"             
## [9] "Paris Basin"                "Sogdiana"                  
##[11] "Susiana"                    "Upper Egypt"  
